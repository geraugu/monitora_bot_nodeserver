...
var Monitora = {
	...
	*/
	buildElements:  function(congressman){
		...
		*/}];
		...
		var mReturn = {
				title: ex+titulo +" "+ congressman.get("nome"),
	...
	*/
	getPolitico: function(params, callback){
		...
		query.limit(10);		
		query.containsAll("words", params.trim().toLowerCase().split(" "));
		/*console.log(params)
		if(params.firstName){
			  ...
			  if(objects.length === 0){
				...
				//json.message = "Não encontrei nenhum parlamentar com esse nome";
					...
					for (var i = 0; i < objects.length; i++) {
						  ...
						  politicos.push({idPolitico: object.id, nome: ex+object.get('nome')})
	...
	getGastaMais: function(params, callback){
		//console.log(params);
		var Politico = Parse.Object.extend("Politico");
		var query = new Parse.Query(Politico);
	...
	getFaltouMais: function(params, callback){
		//console.log(params);
		var Politico = Parse.Object.extend("Politico");
		var query = new Parse.Query(Politico);
		...
		query.find({
		  success: function(objects) {
			...
			//for (var i = 0; i < objects.length; i++) {
				...
				mensagem = object.get("nome")+" ("+object.get("siglaPartido")+"-"+
										object.get("uf")+") Faltou: "+object.get("faltas")+"\n";
				console.log(mensagem)
				mMonitora.getPresenca(object.id, function(ret){
					callback( mensagem+"\n"+ret);
				})
				//}
	...
	*/
	...
	getGastos : function (json, callback){
console.log(json)
//		var json = params[0];
	...
	getPresenca : function (params, callback){
		...
		query.equalTo("politico", "Politico$"+params);
		console.log(params);
		query.find({
		  success: function(objects) {
			// Successfully retrieved the object.
